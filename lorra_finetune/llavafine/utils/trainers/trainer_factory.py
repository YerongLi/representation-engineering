...
class TrainerFactory:
    ...
    TRAINING_ARGS_MAPPING = {
        ...
        'reeng': 'swift.trainers.Seq2SeqTrainingArguments',
    ...
    def get_cls(train_type: str, mapping: Dict[str, str]):
        ...
        if module_path.startswith('.'):
            ...
            # Construct absolute module path based on the current directory
            module_abs_path = os.path.join(current_dir, module_path.lstrip('.')) + '.py'
            
            # Insert the directory containing trainers.py into the system path
            sys.path.insert(0, current_dir)
            
            # Use the module file directly (without importlib, use __import__)
            module = __import__(module_path.lstrip('.'))
        else:
            # Absolute import if the module is not relative
